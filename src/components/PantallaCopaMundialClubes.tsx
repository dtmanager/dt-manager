// Copa Mundial de Clubes de la FIFA (pedido explícito, formato real desde
// 2025): 8 grupos de 4 a una sola rueda (no ida y vuelta, a diferencia de
// Libertadores/Sudamericana), luego octavos de final en adelante a
// partido único hasta la final. Mismo patrón visual que
// PantallaCopaConmebol.tsx, pero con llaves a partido único (como
// PantallaCopaNacional.tsx) en vez de ida y vuelta.
...
export function PantallaCopaMundialClubes({
  ...
  const { copaMundialClubes, clubes: clubesLiga, clubUsuarioId } = useGameStore();
  const [partidoSeleccionado, setPartidoSeleccionado] = useState<Partido | null>(null);
  const [vista, setVista] = useState<'competencia' | 'goleadores'>('competencia');

  // Hooks arriba de cualquier return condicional — copaMundialClubes puede
  // ser null en el primer render.
  const grupos = copaMundialClubes?.grupos ?? [];
  const proximaFecha = copaMundialClubes?.fase === 'grupos' ? proximaFechaDeGrupos(grupos) : null;
  ...
  const ultimosResultados = useMemo(() => {
    if (copaMundialClubes?.fase !== 'grupos' || fechaJugada < 1) return [];
    return grupos.flatMap((g) => g.fixture).filter((p) => p.fecha === fechaJugada && p.golesLocal != null);
  }, [grupos, copaMundialClubes?.fase, fechaJugada]);

  if (!copaMundialClubes) return null;
  const {
    clubes: clubesCopa, clubIds, fase, usuarioEliminado, bracket, campeonId,
  } = copaMundialClubes;
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-200 font-sans p-4 flex flex-col gap-4">
      <header className="flex items-center justify-between topbar-entrada">
        <h1 className="font-bold">Copa Mundial de Clubes de la FIFA</h1>
      ...
      {usuarioEliminado && fase !== 'campeon' && (
        <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-4 text-center text-sm text-neutral-400">
          Quedamos eliminados de la Copa Mundial de Clubes — el resto de la competencia sigue en juego.
        </div>
      )}

      {/* Pedido explícito: "borra todos los simular fecha que no sean el
          de calendario" — esta pantalla pasa a ser de sólo lectura
          (grupos/bracket/goleadores); la simulación vive en Calendario/Hub. */}
